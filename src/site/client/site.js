...
const SITE_TIMING = Object.freeze({
  clipboardTimeoutMs: 800,
  copyFeedbackMs: 1600,
  typeCharacterMs: 72,
  typeLinePauseMs: 5000,
});
      ...
      try {
        await Promise.race([
          ...
          new Promise((_, reject) => window.setTimeout(
            ...
            SITE_TIMING.clipboardTimeoutMs,
          )),
        ]);
      } catch {
  ...
  const showLine = () => {
    ...
    const type = () => {
      ...
      if (index <= activeLine.length) {
        typingTimer = window.setTimeout(type, SITE_TIMING.typeCharacterMs);
      } else {
        if (announcer) announcer.textContent = activeLine;
        nextLineTimer = window.setTimeout(showLine, SITE_TIMING.typeLinePauseMs);
      }
    };
    type();
  };
