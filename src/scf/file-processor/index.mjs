...
async function processDocument(key, metadata, ext) {
  ...
  const request = {
    ...
    ResultType: 9,
    EnableSubImg: false,
    ...(ext === "pdf" ? { PageRange: `1-${metadata.pdfPages || FILE_LIMITS.pdfPages}` } : {}),
  };
...
async function assertPdfPageLimit(key) {
  ...
  if (document.getPageCount() > FILE_LIMITS.pdfPages) throw new Error(`PDF 最多支持 ${FILE_LIMITS.pdfPages} 页`);
}
