...
资料列表按当前目录和资料角色读取，Cloudflare 每次最多从 COS 返回 50 项。客户端收到首批结果后立即展示，再顺序加载后续分页并按路径去重；切换专题、资料角色或目录会取消旧分页请求，避免结果写入错误视图。普通列表不会递归读取文件夹内的元数据，也不会同步计算研报文件夹的“已纳入/总数”。

研报文件夹的纳入统计仅在管理员打开“批量管理”时执行。受 Cloudflare 单次 50 个外部子请求上限约束，`GET /api/drive/folder` 每页扫描最多 40 份元数据，客户端逐页累计并显示进度；统计完成后，管理员可明确选择“全部纳入”或“全部取消”。`PATCH /api/drive/folder` 每页最多处理 20 项，为每项预留一次读取和一次写入，并给专题及 COS 列表请求留出余量；单项失败不会中断该页或后续分页，重复执行对已完成项保持幂等。
